...
def order_edit(frame):
    ...
    while editing == "yes":
        ...
        frame = indexing(new_order_list, new_total_list)
        print(frame)
...
def document(to_print, frame):
    ...
    filename = "MMF_{}_{}_{} - {}".format(year, month, day, order_count)
...
while ordering == "yes":
    ...
    if edit == "yes":
        pizza_frame = order_edit(pizza_frame)
    ...
    ordering = string_checker("Would you like to place another order? ", 2, yes_no_list)
    print_list.clear()
    total_p.clear()
    order_count += 1
    print()
